...
class SimpleChain {
  //this should prob just be a single 'config' object, instead of tons of args
		...
		if(newChain){
			...
			fs.readdir(this.blockFolderRoot, (err, files) => {
			  ...
			  for (const file of files) {
				rimraf(this.blockFolderRoot + file);
			  }
			});
			...
			fs.readdir(treesDir, (err, files) => {
			  ...
			  for (const file of files) {
				fs.unlink(path.join(treesDir, file), err => {
				  if (err) throw err;
				});
			  }
			});
	...
	RecordBlock(){
		...
		const currentBlockHash = this.web3.utils.sha3(JSON.stringify([this.currentBlock, this.blockCount]));

		const thisMerkleHash = this.web3.utils.sha3(JSON.stringify([currentBlockHash, this.lastMerkleHash, this.blockCount]));
	...
	RecordSnapshot(){
		const snapshotHash = this.web3.utils.sha3(JSON.stringify([this.lastSnapshotHash, this.lastMerkleHash, this.snapshotCount]));
